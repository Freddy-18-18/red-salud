...
-- Índices
CREATE INDEX idx_chat_workspaces_owner ON chat_workspaces(owner_id);
CREATE INDEX idx_chat_workspaces_type ON chat_workspaces(workspace_type);
CREATE INDEX idx_chat_workspaces_public ON chat_workspaces(is_public) WHERE is_public = true;
...
CREATE INDEX idx_chat_categories_workspace ON chat_categories(workspace_id, position);
...
CREATE INDEX idx_chat_channels_workspace ON chat_channels(workspace_id) WHERE workspace_id IS NOT NULL;
CREATE INDEX idx_chat_channels_category ON chat_channels(category_id) WHERE category_id IS NOT NULL;
CREATE INDEX idx_chat_channels_entity ON chat_channels(primary_entity_type, primary_entity_id) WHERE primary_entity_id IS NOT NULL;
CREATE INDEX idx_chat_channels_archived ON chat_channels(is_archived) WHERE is_archived = false;
CREATE INDEX idx_chat_channels_last_message ON chat_channels(last_message_at DESC) WHERE last_message_at IS NOT NULL;
...
CREATE INDEX idx_chat_participants_presence ON chat_participants(user_id, presence_status) WHERE presence_status != 'offline';
CREATE INDEX idx_chat_participants_unread ON chat_participants(user_id, channel_id, last_read_at);
...
CREATE INDEX idx_chat_messages_channel_created ON chat_messages(channel_id, created_at DESC) WHERE deleted_at IS NULL;
CREATE INDEX idx_chat_messages_thread ON chat_messages(thread_root_id, created_at) WHERE thread_root_id IS NOT NULL;
...
CREATE INDEX idx_chat_messages_delivery ON chat_messages(channel_id, sender_id, delivery_status);
CREATE INDEX idx_chat_messages_phi ON chat_messages(is_phi) WHERE is_phi = true;
CREATE INDEX idx_chat_messages_type ON chat_messages(message_type);

-- Índices GIN para búsqueda JSON
CREATE INDEX idx_chat_messages_reactions ON chat_messages USING GIN(reactions);
CREATE INDEX idx_chat_messages_rich_content ON chat_messages USING GIN(rich_content);
CREATE INDEX idx_chat_messages_medical_context ON chat_messages USING GIN(medical_context);
...
CREATE INDEX idx_chat_message_reads_user ON chat_message_reads(user_id, read_at DESC);
...
CREATE INDEX idx_chat_typing_expiration ON chat_typing_indicators(last_typing_at);
...
CREATE INDEX idx_chat_presence_status ON chat_user_presence(presence_status) WHERE presence_status != 'offline';
CREATE INDEX idx_chat_presence_expires ON chat_user_presence(expires_at) WHERE expires_at IS NOT NULL;
