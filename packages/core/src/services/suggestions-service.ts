...
import { CONSULTATION_REASONS } from '../data/consultation-reasons';
import {
...
// TODO: getTopReasons is not exported from the original module - inlined helper
function getTopReasons(specialty: string, limit: number): string[] {
    return getReasonsBySpecialty(specialty).slice(0, limit).map(r => r.reason);
}
