...
export async function resolveSacsToSlug(
  ...
  if (keywordMatches.length > 0) {
    return {
      slug: keywordMatches[0].specialty_slug,
      confidence: "keyword",
      matchedPattern: keywordMatches[0].sacs_name_pattern,
    };
  }
  ...
  if (fuzzyMatch.length > 0) {
    return {
      slug: fuzzyMatch[0].specialty_slug,
      confidence: "exact",
      matchedPattern: fuzzyMatch[0].sacs_name_pattern,
    };
  }
