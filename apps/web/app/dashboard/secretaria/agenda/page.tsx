...
interface AppointmentQueryResult {
  ...
  motivo: string | null;
  status: CalendarAppointment["status"] | null;
  tipo_cita: CalendarAppointment["tipo_cita"] | null;
  color: string | null;
...
export default function SecretariaAgendaPage() {
    ...
    try {
      ...
      if (data) {
        const formattedAppointments: CalendarAppointment[] = (
          data as unknown as AppointmentQueryResult[]
        ).map((apt) => {
          const isOfflinePatient = !apt.paciente_id && apt.offline_patient_id;
          const patientData = isOfflinePatient
            ? apt.offline_patient
            : apt.paciente;
          ...
          return {
            ...
            paciente_avatar: isOfflinePatient
              ? null
              : apt.paciente?.avatar_url || null,
            fecha_hora: apt.fecha_hora,
            fecha_hora_fin: new Date(
              new Date(apt.fecha_hora).getTime() +
              (apt.duracion_minutos || 30) * 60000
            ).toISOString(),
  ...
  const handleMessage = (appointment: CalendarAppointment) => {
    ...
    router.push(
      `/dashboard/secretaria/mensajes?patient=${appointment.paciente_id}`
    );
  };

  const handleStartVideo = () => {
    alert(
      "Las secretarias no pueden iniciar videollamadas. Solo el médico puede hacerlo."
    );
  };
