...
const formSchema = z.object({
    ...
    payment_date: z.date({ message: "Fecha requerida" }),
});
...
export function PagoMovilForm({ onSubmit, isLoading }: PagoMovilFormProps) {
    const form = useForm<{ amount: number; reference_number: string; bank_origin: string; payment_date: Date }>({
    ...
    const handleSubmit = async (values: { amount: number; reference_number: string; bank_origin: string; payment_date: Date }) => {
        await onSubmit({
            ...values,
            currency: "VES",
