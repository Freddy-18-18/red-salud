...
export function buildSpecialtyConfig(
  ...
  // Nota: no todas las especialidades tienen rutas dedicadas en el App Router.
  // Para las que no existen, los módulos deben apuntar a rutas compartidas
  // (ej: /dashboard/medico/recetas) y NO a /dashboard/medico/{slug}/recetas.
  // Mantener este set alineado con las carpetas reales en apps/medico/web/src/app/dashboard/medico/*.
  const SPECIALTIES_WITH_DEDICATED_DASHBOARD_ROUTES = new Set([
    'cardiologia',
    'ginecologia',
    'neurologia',
    'odontologia',
  ...
  const defaultBasePath = SPECIALTIES_WITH_DEDICATED_DASHBOARD_ROUTES.has(identity.slug)
    ? `/dashboard/medico/${identity.slug}`
    : '/dashboard/medico';

  const basePath = override?.dashboardPath ?? defaultBasePath;
