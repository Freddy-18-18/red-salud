    ...
    if (userRole === 'secretaria') {
      ...
      const baseItems = [
        ...
        },
        {
          ...
          label: 'Estadísticas',
          icon: 'Activity',
          route: '/consultorio/estadisticas',
        },
      ];

      // Non-dental specific items
      // For Urology, we also skip these as they have specialized replacements
      if (!isDental && specialtyConfig.slug !== 'urologia') {
      ...
      // Para Urología: aplanar todos los grupos en uno solo (sin cabeceras de grupo)
      if (specialtyConfig.slug === 'urologia') {
        const allSpecialtyItems = specialtyMenuGroups.flatMap((g) => g.items);
        menuGroups = [
          {
            label: 'Principal',
            items: [...baseItems, ...allSpecialtyItems],
          },
        ];
      } else {
