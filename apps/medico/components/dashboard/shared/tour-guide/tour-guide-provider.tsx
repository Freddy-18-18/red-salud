...
export function TourGuideProvider({ children }: TourGuideProviderProps) {
  ...
  useEffect(() => {
    ...
    if (isFirstVisit) {
      ...
      // Buscar tour con autoStart para la ruta actual
      const tourForRoute = getTourForRoute(pathname);
      const autoTour = tourForRoute && tourForRoute.autoStart && !completedTours.includes(tourForRoute.id);

      if (autoTour && tourForRoute) {
        // Delay para que la UI esté lista
        setTimeout(() => {
          startTour(tourForRoute.id);
        }, 1500);
      }
    }
  }, [settings.autoStartTours, completedTours, startTour, pathname]);
