...
export function PatientQuickActions({
  ...
  const handleAction = (action: string) => {
    switch (action) {
      case "view":
        if (isOffline) {
          router.push(`/consultorio/pacientes/offline/${patientId}`);
        } else {
          router.push(`/consultorio/pacientes/${patientId}`);
        }
        break;
      case "message":
        if (!isOffline) {
          router.push(`/consultorio/mensajeria?patient=${patientId}`);
        }
        break;
      case "appointment":
        if (!isOffline) {
          router.push(`/consultorio/citas/nueva?patient=${patientId}`);
        }
        break;
      case "prescription":
        if (!isOffline) {
          router.push(`/consultorio/recetas/nueva?patient=${patientId}`);
        }
        break;
      case "consultation":
        if (!isOffline) {
          router.push(`/consultorio/consultas/nueva?patient=${patientId}`);
        }
        break;
      case "call":
      ...
      case "video":
        if (!isOffline) {
          router.push(`/consultorio/videollamada?patient=${patientId}`);
        }
        break;
    }
  };
