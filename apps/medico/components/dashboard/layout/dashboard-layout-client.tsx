...
export function DashboardLayoutClient({
  ...
  // Hooks para médicos
  const { profile: doctorProfileRaw, loading: profileLoading } = useDoctorProfile(
    userRole === "medico" ? userId : undefined
  );

  // Transformar datos del perfil para el header
  const doctorProfile = doctorProfileRaw ? {
    nombre_completo: doctorProfileRaw.profile?.nombre_completo || doctorProfileRaw.nombre_completo || "",
    specialty: doctorProfileRaw.specialty ? { name: doctorProfileRaw.specialty.name } : undefined,
    sacs_especialidad: doctorProfileRaw.profile?.sacs_especialidad || doctorProfileRaw.sacs_especialidad || "",
    is_verified: doctorProfileRaw.is_verified || doctorProfileRaw.verified || false,
    sacs_verified: doctorProfileRaw.sacs_verified || doctorProfileRaw.profile?.sacs_verificado || false,
    verification_status: doctorProfileRaw.verification_status || "pending",
  } : null;
  ...
  const handleProfileClick = () => {
    if (userRole === "medico") {
      ...
      router.push("/consultorio/configuracion");
    } else {
