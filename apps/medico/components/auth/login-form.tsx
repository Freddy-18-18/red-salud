...
import { APP_URLS, getAppUrl } from "@/lib/app-urls";
...
function LoginFormContent({ role, roleLabel }: LoginFormProps) {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    resetAttempts();
    const userRole = result.user.user_metadata?.role || "medico";
    ...
    <div className="h-screen bg-background relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 px-4 pt-4 pb-2 shrink-0 flex items-center justify-between">
        ...
        >
          ...
          <span className="text-xs sm:text-sm font-bold">Portal Red-Salud</span>
        </a>
      </div>
          ...
          >
            ...
            <Card className="border shadow-glow bg-card/40 backdrop-blur-xl overflow-hidden border-white/10 dark:border-white/5">
              <CardContent className="p-5 sm:p-6">
                ...
                <div className="mt-5 sm:mt-6 space-y-2 text-center text-xs sm:text-sm text-muted-foreground">
                  <p>
                    ¿No tienes cuenta?{" "}
                    <Link
                      href="/register"
