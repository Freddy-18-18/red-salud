...
import type { OfficeTabId } from "@/components/dashboard/medico/configuracion/offices-section";

const LoadingFallback = () => <div className="w-full h-[400px] rounded-xl bg-gray-100 dark:bg-black/80 animate-pulse" />;
...
function ConfiguracionContent() {
  ...
  let tabFromUrl = searchParams?.get("tab") || "perfil";

  if (tabFromUrl === "configuracion-recetas") {
    tabFromUrl = "recetas";
  }

  const officeTabFromUrl = (searchParams?.get("officeTab") as OfficeTabId | null) ?? undefined;
  const initialOfficeTab: OfficeTabId = tabFromUrl === "horarios" ? "horarios" : (officeTabFromUrl ?? "general");
  if (tabFromUrl === "horarios") {
    tabFromUrl = "consultorios";
  }

  const activeTab = TABS.some(t => t.id === tabFromUrl) ? tabFromUrl : "perfil";
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "consultorios": return <OfficesSection initialTab={initialOfficeTab} />;
          ...
          <div className="w-full h-full px-8 py-8">
            <div className="relative">
              <motion.div
                ...
                className="bg-white/90 dark:bg-black/90 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-white/10 shadow-xl shadow-gray-200/20 dark:shadow-none overflow-hidden"
              >
                <div className="p-10">
                  {renderTabContent()}
                </div>
