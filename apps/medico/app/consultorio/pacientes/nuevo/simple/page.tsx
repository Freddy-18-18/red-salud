...
import { patientSchema, type PatientFormValues } from "@red-salud/core/validations";
  ...
  const onSubmit = async (data: PatientFormValues) => {
    ...
    try {
      ...
      if (existingPatient) {
        ...
        if (fromCita) {
          router.push(`/consultorio/citas/nueva?paciente=${existingPatient.id}`);
        } else {
          router.push(`/consultorio/pacientes/${existingPatient.id}`);
        }
        return;
      }
      ...
      // Redirigir según de dónde viene
      if (fromCita) {
        router.push(`/consultorio/citas/nueva?paciente=${offlinePatient.id}`);
      } else {
        router.push(`/consultorio/pacientes/offline/${offlinePatient.id}`);
      }
    } catch (err) {
