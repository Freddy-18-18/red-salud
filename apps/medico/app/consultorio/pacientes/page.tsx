...
import { Toast, type ToastType } from "@red-salud/design-system";
...
export default function DoctorPatientsPage() {
  ...
  const handleView = useCallback((p: RegisteredPatient | OfflinePatient) => {
    ...
    if (isRegistered) {
      ...
      router.push(`/consultorio/pacientes/${rp.patient_id}`);
    } else {
      const op = p as OfflinePatient;
      router.push(`/consultorio/pacientes/offline/${op.id}`);
    }
  }, [router]);

  const handleMessage = useCallback((p: RegisteredPatient) => {
    router.push(`/consultorio/mensajeria?patient=${p.patient_id}`);
  }, [router]);
  ...
  const handleStartConsultation = useCallback(async (appointment: TodayAppointment) => {
    ...
    try {
      ...
      if (appointment.status === "en_consulta") {
        ...
        router.push(`/consultorio/pacientes/consulta?${params.toString()}`);
        return;
      }
  ...
  const handleRegisterPatient = useCallback(() => {
    ...
    router.push("/consultorio/pacientes/nuevo");
  }, [state.selectedOfficeId, router]);

  return (
    <VerificationGuard>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 dark:from-black dark:via-black dark:to-black">
        <Toast
        ...
        <div className="container mx-auto px-4 py-8 space-y-8">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-gray-600 dark:text-white/60 mt-0.5">
                      {state.loading ? (
          ...
          {expandedSection === "all" && (
            <div className="animate-in slide-in-from-top-2 duration-300 space-y-4">
              <Card className="border-0 shadow-lg shadow-gray-200/50 dark:shadow-none bg-white/80 dark:bg-black/80 backdrop-blur-sm">
                  ...
                  <>
                    ...
                    {!state.loading && (
                      <Pagination
                        ...
                        className="mt-6 pt-4 border-t border-gray-100 dark:border-white/10"
                      />
                    )}
                  </>
                ) : (
                  <Card className="border-0 shadow-lg bg-white/80 dark:bg-black/80 backdrop-blur-sm">
                    <CardContent className="py-16">
                      <div className="text-center">
                        ...
                        {!state.searchQuery && (
                          <Button
                            onClick={() => router.push("/consultorio/pacientes/nuevo")}
