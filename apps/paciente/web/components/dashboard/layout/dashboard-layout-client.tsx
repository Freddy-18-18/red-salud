...
import { PacienteDashboardHeader } from "./paciente-dashboard-header";
...
import { usePacienteConfigNav } from "@/hooks/dashboard/use-paciente-config-nav";
...
export function DashboardLayoutClient({
  ...
  // Hook de navegación de configuración para pacientes (activo solo en rutas /configuracion)
  const pacienteConfigNav = usePacienteConfigNav();

  /**
   * Maneja el clic en el perfil del usuario.
   * Navega a la página de configuración según el rol.
   */
  const handleProfileClick = () => {
    if (userRole === "medico") {
      router.push("/dashboard/medico/configuracion");
    } else {
      router.push("/dashboard/paciente/configuracion");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Paciente Header */}
      {userRole === "paciente" && (
        <PacienteDashboardHeader
          patientProfile={{ nombre_completo: userName }}
          onChatClick={() => {
            document.dispatchEvent(new CustomEvent("toggle-chat"));
          }}
          onHelpClick={() => startTour("dashboard-overview")}
          configNav={pacienteConfigNav}
          className="hidden md:flex"
        />
      )}
