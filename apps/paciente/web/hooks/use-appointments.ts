...
import {
  ...
  type TimeSlot,
} from "@red-salud/sdk-paciente";

import { supabase } from "@/lib/supabase/client";

const pacienteSdk = createPacienteSdk(supabase);
...
export function usePatientAppointments(patientId: string | undefined) {
  ...
  const refreshAppointments = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await pacienteSdk.appointments.getPatientAppointments(patientId);
...
export function useDoctorAppointments(doctorId: string | undefined) {
  ...
  const refreshAppointments = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await pacienteSdk.appointments.getDoctorAppointments(doctorId);
      setAppointments(data);
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Error loading appointments');
    } finally {
...
export function useMedicalSpecialties(onlyWithDoctors: boolean = false) {
  ...
  useEffect(() => {
    const loadSpecialties = async () => {
      try {
        const data = await pacienteSdk.appointments.getMedicalSpecialties(onlyWithDoctors);
        setSpecialties(data);
      } catch (error) {
...
export function useAvailableDoctors(specialtyId?: string) {
  ...
  useEffect(() => {
    const loadDoctors = async () => {
      setLoading(true);
      try {
        const data = await pacienteSdk.appointments.getAvailableDoctors(specialtyId);
        setDoctors(data);
      } finally {
        setLoading(false);
      }
...
export function useDoctorProfile(doctorId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const loadDoctor = async () => {
      setLoading(true);
      try {
        const data = await pacienteSdk.appointments.getDoctorProfile(doctorId);
        setDoctor(data);
      } finally {
        setLoading(false);
      }
...
export function useAvailableTimeSlots(
  ...
  useEffect(() => {
    const loadTimeSlots = async () => {
      ...
      setLoading(true);
      try {
        const data = await pacienteSdk.appointments.getAvailableTimeSlots(doctorId, date);
        setTimeSlots(data);
      } finally {
        setLoading(false);
      }
...
export function useCreateAppointment() {
  ...
  const create = async (
    ...
    setError(null);
    try {
      return await pacienteSdk.appointments.createAppointment(patientId, appointmentData);
    } catch (error) {
...
export function useCancelAppointment() {
  ...
  const cancel = async (
    ...
    setError(null);
    try {
      await pacienteSdk.appointments.cancel({ appointmentId, reason });
