...
export default function InventarioPage() {
  ...
  const loadExchangeRate = async () => {
    try {
      // TODO: Use BCVRateFetcher from @red-salud/core once extracted
      setExchangeRate(36);
    } catch (error) {
  ...
  const getExpiryStatus = (fecha: string) => {
    ...
    if (daysUntilExpiry <= 30) return { status: 'warning', color: 'bg-orange-100 text-orange-800', label: `${daysUntilExpiry} dias` };
    if (daysUntilExpiry <= 90) return { status: 'caution', color: 'bg-yellow-100 text-yellow-800', label: `${daysUntilExpiry} dias` };
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="md:col-span-2">
                <div className="relative">
                  ...
                  <Input
                    placeholder="Buscar por nombre, descripcion o laboratorio..."
                    value={searchTerm}
