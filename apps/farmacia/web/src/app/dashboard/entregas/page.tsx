...
export default function EntregasPage() {
  ...
  const getEstadoColor = (estado: string) => {
    switch (estado) {
      ...
      case "fallida": return "bg-red-200 text-red-900";
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Gestion de entregas</p>
            </div>
            <Button size="sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <div className="relative">
                  ...
                  <Input
                    placeholder="Buscar por cliente, cedula o tracking..."
                    value={searchTerm}
                      ...
                      <TableRow key={entrega.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            {new Date(entrega.fecha_programada).toLocaleString("es-VE")}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                        ...
                        <TableCell>
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${getEstadoColor(entrega.estado)}`}
                          >
                            {entrega.estado}
                          </span>
                        </TableCell>
                        <TableCell>
                          <div className="flex gap-1">
                            {entrega.estado === "pendiente" && (
                              <Button variant="ghost" size="sm" onClick={() => actualizarEstado(entrega.id, "en_ruta")}>
                                <Truck className="h-4 w-4 text-blue-600" />
                              </Button>
                            )}
                            {entrega.estado === "en_ruta" && (
                              <Button variant="ghost" size="sm" onClick={() => actualizarEstado(entrega.id, "entregada")}>
                                <CheckCircle className="h-4 w-4 text-green-600" />
                              </Button>
                            )}
                            {(entrega.estado === "pendiente" || entrega.estado === "en_ruta") && (
                              <Button variant="ghost" size="sm" onClick={() => actualizarEstado(entrega.id, "cancelada")}>
