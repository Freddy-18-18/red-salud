/**
 * Servicio para gestion de operaciones clinicas
 *
 * Maneja recursos, turnos, metricas y alertas operacionales
 */
  ...
  for (const locationId of locationIds) {
    ...
    if (totalBeds > 0 && availableBeds / totalBeds < 0.2) {
      alerts.push({
        ...
        type: 'capacity',
        severity: 'critical',
        title: 'Capacidad Critica de Camas',
    ...
    if (operatingRooms.length > 0 && availableOR === 0) {
      alerts.push({
        ...
        type: 'operational',
        severity: 'warning',
        title: 'Sin Quirofanos Disponibles',
        message: 'Todos los quirofanos estan ocupados o en mantenimiento',
        location_id: locationId,
        created_at: new Date().toISOString(),
        resolved: false,
      });
    ...
    for (const metric of metrics.data) {
      ...
      if (metric.occupancy_rate && metric.occupancy_rate < 50) {
        alerts.push({
          ...
          type: 'operational',
          severity: 'info',
          title: 'Baja Ocupacion',
          message: `Ocupacion del ${metric.occupancy_rate.toFixed(1)}% hoy`,
          location_id: metric.location_id,
          created_at: new Date().toISOString(),
          resolved: false,
        });
