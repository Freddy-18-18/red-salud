/**
 * Servicio para gestion de pacientes internacionales
 *
...
export async function getInternationalPatientStats(clinicId: string): Promise<{
  ...
  const pendingArrivalsData = await getPendingArrivals(clinicId, 30);
  ...
  return {
    ...
    pending_arrivals: pendingArrivalsData.length,
...
export async function getCountryRequirements(countryCode: string): Promise<{
  ...
  const requirements: Record<string, Record<string, unknown>> = {
    USA: {
      ...
      notes: 'Se requiere visa de turismo medico o visa B-2',
    },
    CAN: {
